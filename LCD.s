...
;6) If bit 4 is high, loop back to step 5 (wait for BUSY bit to be low)
	PUSH {R4-R11}					;Saves All The Registers
	LDR R3, =SSI0_SR_R
POLL
	LDR R4, [R3]
	AND R4, R4, #0x10
	CMP R4, #0
	BNE POLL                        ;Polls To Check If SSI0_SR_R Bit 4 Is Low
	LDR R5, =DC
	LDR R6, =DC_COMMAND
	STR R6, [R5]					;Sets DC To 0
	LDR R5, =SSI0_DR_R
	STRB R0, [R5]					;Writes Command in R0 To SSI0_DR_R
POLL2
	LDR R4, [R3]
	AND R4, R4, #0x10
	CMP R4, #0
	BNE POLL2						;Polls To Check If SSI0_SR_R Bit 4 Is Low
    POP {R4-R11}					;Restores All Of The Registers
    BX  LR                          ;Return
...
POLL3
	...
	AND R4, R4, #0x01
	CMP R4, #0
	BEQ POLL3                        ;Polls To Check If SSI0_SR_R Bit 1 Is High
 	LDR R5, =DC
	LDR R6, =DC_DATA
	STR R6, [R5]					;Sets DC To DC_Data Which Is 0x40
	LDR R5, =SSI0_DR_R
	STRB R0, [R5]					;Writes Data in R0 To SSI0_DR_R
    POP {R4-R11}					;Restores All Of The Registers
    BX  LR                          ;Return
